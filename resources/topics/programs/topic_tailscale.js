...
registerTopic({
    ...
    hero: {
        ...
        introDe: 'Tailscale ist eine <strong>Zero-Trust identity-basierte Konnektivitätsplattform</strong>, die Ihr Legacy-VPN, SASE und PAM ersetzt und Remote-Teams, Multi-Cloud-Umgebungen, CI/CD-Pipelines, Edge- und IoT-Geräte sowie KI-Workloads verbindet[citation:1]. Basierend auf dem <strong>WireGuard-Protokoll</strong> ermöglicht es verschlüsselte Punkt-zu-Punkt-Verbindungen ohne komplexe Netzwerkkonfiguration. <a href="https://tailscale.com/docs/" target="_blank">Zur offiziellen Dokumentation</a>.',
        introEn: 'Tailscale is a <strong>Zero Trust identity-based connectivity platform</strong> that replaces your legacy VPN, SASE, and PAM and connects remote teams, multi-cloud environments, CI/CD pipelines, Edge & IoT devices, and AI workloads[citation:1]. Built on the <strong>WireGuard protocol</strong>, it enables encrypted point-to-point connections without complex network configuration. <a href="https://tailscale.com/docs/" target="_blank">Visit the official documentation</a>.'
    },
    ...
    sections: [
        /* ============ 1. ÜBERBLICK ============ */
        {
            ...
            introDe: 'Tailscale implementiert ein <strong>Mesh-Netzwerk</strong> (auch bekannt als "tailnet"), in dem jedes Gerät direkt mit jedem anderen kommunizieren kann. Der zentrale Koordinationsserver verwaltet die Netzwerktopologie und Schlüssel, transportiert aber <strong>keinen Nutzdatenverkehr</strong>. Die eigentliche Datenübertragung erfolgt direkt zwischen den Geräten über WireGuard. Mehr dazu in den <a href="https://tailscale.com/docs/concepts" target="_blank">technischen Übersichten</a>.',
            introEn: 'Tailscale implements a <strong>mesh network</strong> (also known as a "tailnet") where every device can communicate directly with every other device. The central coordination server manages network topology and keys but <strong>carries no user traffic</strong>. Actual data transfer happens directly between devices over WireGuard. Learn more in the <a href="https://tailscale.com/docs/concepts" target="_blank">technical overviews</a>.',
            subtopics: [
                {
                    id: 'subsection1_1',
                    ...
                    <tr><td><strong>Typ</strong></td><td class="text-[var(--text-muted)]">Zero-Trust Identity-basierte Mesh-VPN-Plattform.</td></tr>
                    <tr><td><strong>Protokoll</strong></td><td class="text-[var(--text-muted)]">WireGuard (UDP, modern, kryptographisch sicher).</td></tr>
                    ...
                    <tr><td><strong>NAT-Traversal</strong></td><td class="text-[var(--text-muted)]">Automatisch via DERP-Relays und STUN. Keine Portfreigaben nötig.</td></tr>
                    <tr><td><strong>Plattformen</strong></td><td class="text-[var(--text-muted)]">Linux, Windows, macOS, iOS, Android, tvOS, Docker, Kubernetes.</td></tr>
                    <tr><td><strong>Kernfunktion</strong></td><td class="text-[var(--text-muted)]">Sichere Verbindung zwischen allen Geräten, Subnet-Routing, Exit Nodes, SSH.</td></tr>
                    ...
                    <tr><td><strong>Type</strong></td><td class="text-[var(--text-muted)]">Zero Trust identity-based mesh VPN platform.</td></tr>
                    <tr><td><strong>Protocol</strong></td><td class="text-[var(--text-muted)]">WireGuard (UDP, modern, cryptographically secure).</td></tr>
                    ...
                    <tr><td><strong>NAT traversal</strong></td><td class="text-[var(--text-muted)]">Automatic via DERP relays and STUN. No port forwarding required.</td></tr>
                    <tr><td><strong>Platforms</strong></td><td class="text-[var(--text-muted)]">Linux, Windows, macOS, iOS, Android, tvOS, Docker, Kubernetes.</td></tr>
                    <tr><td><strong>Core function</strong></td><td class="text-[var(--text-muted)]">Secure connection between all devices, subnet routing, exit nodes, SSH.</td></tr>
                ...
                },
                {
                    ...
                    <li><strong>DERP (Designated Encrypted Relay for Packets):</strong> Relay-Server, die zum Einsatz kommen, wenn eine direkte Verbindung nicht möglich ist (z.B. bei symmetrischem NAT). Die Ende-zu-Ende-Verschlüsselung bleibt erhalten.</li>
                    <li><strong>STUN:</strong> Wird für NAT-Traversal verwendet, um die öffentliche IP und den Port eines Geräts zu ermitteln.</li>
                    <li><strong>WireGuard:</strong> Das zugrundeliegende VPN-Protokoll. Schnell, modern und im Linux-Kernel integriert.</li>
                    </ul>
                    <p class="mt-3"><strong>Mental Model:</strong> Stellen Sie sich ein privates U-Bahn-System vor. Jede Station (Gerät) kennt jede andere. Züge (Pakete) fahren direkt zwischen den Stationen. Der Netzplan (Koordinationsserver) wird zentral verwaltet, befördert aber keine Passagiere.[citation:18]</p>
                    </div>
                    `,
                    htmlEn: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    ...
                    <li><strong>DERP (Designated Encrypted Relay for Packets):</strong> Relay servers used when a direct connection is not possible (e.g., symmetric NAT). End-to-end encryption is preserved.</li>
                    <li><strong>STUN:</strong> Used for NAT traversal to discover a device's public IP and port.</li>
                    <li><strong>WireGuard:</strong> The underlying VPN protocol. Fast, modern, and integrated into the Linux kernel.</li>
                    </ul>
                    <p class="mt-3"><strong>Mental model:</strong> Think of it as your own private subway system. Every station (device) knows where every other station is. Trains (packets) travel directly between stations. The map (coordination server) is maintained centrally but carries zero passengers.[citation:18]</p>
                    </div>
                    `
                }
            ]
        ...
        /* ============ 2. INSTALLATION ============ */
        {
            ...
            introDe: 'Tailscale ist für praktisch alle gängigen Plattformen verfügbar. Die Installation erfolgt über Paketmanager, Installer oder Docker-Images. Nach der Installation wird <code>tailscale up</code> ausgeführt, um sich im Netzwerk zu authentifizieren. Weitere Details in der <a href="https://tailscale.com/download" target="_blank">offiziellen Download-Seite</a>.',
            introEn: 'Tailscale is available for virtually all common platforms. Installation is done via package managers, installers, or Docker images. After installation, run <code>tailscale up</code> to authenticate with the network. See the <a href="https://tailscale.com/download" target="_blank">official download page</a> for details.',
            subtopics: [
                {
                    id: 'subsection2_1',
                    ...
                    <tr><td><strong><a href="https://tailscale.com/download/linux" target="_blank">Linux</a></strong></td><td class="text-[var(--text-muted)]">Offizielle Repositories für Debian/Ubuntu, Fedora, Arch, openSUSE. Installation via <code>apt</code>, <code>dnf</code>, <code>pacman</code>, <code>zypper</code>.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/windows" target="_blank">Windows</a></strong></td><td class="text-[var(--text-muted)]">Installer (MSI) oder portable Version. GUI und CLI verfügbar.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/mac" target="_blank">macOS</a></strong></td><td class="text-[var(--text-muted)]">DMG-Installer oder Mac App Store. Native GUI-Integration.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/ios" target="_blank">iOS / iPadOS</a></strong></td><td class="text-[var(--text-muted)]">App Store. Volle Integration mit iOS-Netzwerkerweiterungen.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/android" target="_blank">Android</a></strong></td><td class="text-[var(--text-muted)]">Google Play oder F-Droid. VPN-Dienst-Integration.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/docker" target="_blank">Docker</a></strong></td><td class="text-[var(--text-muted)]">Offizielles Image <code>tailscale/tailscale</code>. Für Container-Workloads.</td></tr>
                    </table>
                    </div>
                    `,
                    htmlEn: `
                    ...
                    <tr><td><strong><a href="https://tailscale.com/download/linux" target="_blank">Linux</a></strong></td><td class="text-[var(--text-muted)]">Official repos for Debian/Ubuntu, Fedora, Arch, openSUSE. Install via <code>apt</code>, <code>dnf</code>, <code>pacman</code>, <code>zypper</code>.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/windows" target="_blank">Windows</a></strong></td><td class="text-[var(--text-muted)]">Installer (MSI) or portable version. GUI and CLI available.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/mac" target="_blank">macOS</a></strong></td><td class="text-[var(--text-muted)]">DMG installer or Mac App Store. Native GUI integration.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/ios" target="_blank">iOS / iPadOS</a></strong></td><td class="text-[var(--text-muted)]">App Store. Full integration with iOS network extensions.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/android" target="_blank">Android</a></strong></td><td class="text-[var(--text-muted)]">Google Play or F-Droid. VPN service integration.</td></tr>
                    <tr><td><strong><a href="https://tailscale.com/download/docker" target="_blank">Docker</a></strong></td><td class="text-[var(--text-muted)]">Official image <code>tailscale/tailscale</code>. For container workloads.</td></tr>
                    </table>
                    </div>
                    `
                },
                {
                    ...
                    <p class="mt-3">Nach dem Start wird ein Authentifizierungs-URL ausgegeben. Diese im Browser öffnen und mit dem Tailscale-Konto anmelden. Weitere Details in der <a href="https://tailscale.com/docs/install/linux" target="_blank">offiziellen Linux-Installationsanleitung</a>.</p>
                    </div>
                    `,
                    htmlEn: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    ...
                    <p class="mt-3">After starting, an authentication URL is displayed. Open it in a browser and sign in with your Tailscale account. See the <a href="https://tailscale.com/docs/install/linux" target="_blank">official Linux installation guide</a> for details.</p>
                    </div>
                    `
                }
            ]
        ...
        /* ============ 3. SUBNET ROUTER ============ */
        {
            ...
            introDe: 'Ein <strong>Subnet Router</strong> ist ein Gerät in Ihrem lokalen Netzwerk, das Tailscale ausführt und das lokale Subnetz für Ihr Tailnet bewirbt. Dadurch können alle Geräte in Ihrem Tailnet auf jedes IP-Adresse im beworbenen Subnetz zugreifen – ohne dass auf den Zielgeräten Tailscale installiert sein muss[citation:15].',
            introEn: 'A <strong>subnet router</strong> is a device on your local network that runs Tailscale and advertises the local subnet to your tailnet. This allows any device on your tailnet to reach every IP address on that subnet – without requiring Tailscale to be installed on the target devices[citation:15].',
            subtopics: [
                {
                    id: 'subsection3_1',
                    ...
                    htmlDe: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    <p class="mb-3"><strong>Schritte auf Linux:</strong></p>
...
sudo sysctl -p /etc/sysctl.d/99-tailscale.conf</pre>
                    ...
                    <li>Zur <a href="https://console.tailscale.com/admin/machines" target="_blank">Machines-Seite</a> navigieren.</li>
                    <li>Das Gerät mit dem <strong>Subnets</strong>-Badge finden.</li>
                    <li>Menü öffnen → <strong>Edit route settings</strong> → Routen aktivieren → <strong>Save</strong>[citation:4].</li>
                    </ul>
                    ...
                    htmlEn: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    <p class="mb-3"><strong>Steps on Linux:</strong></p>
                    <ol class="list-decimal pl-5 space-y-2 mb-3">
...
sudo sysctl -p /etc/sysctl.d/99-tailscale.conf</pre>
                    ...
                    <ul class="list-disc pl-4 space-y-1 mt-1">
                    <li>Go to the <a href="https://console.tailscale.com/admin/machines" target="_blank">Machines page</a>.</li>
                    <li>Find the device with the <strong>Subnets</strong> badge.</li>
                    <li>Open menu → <strong>Edit route settings</strong> → enable routes → <strong>Save</strong>[citation:4].</li>
                    </ul>
                ...
                },
                {
                    ...
                    <p class="mt-3">Der gesamte Internet-Traffic des Clients wird nun über den Exit Node geroutet. Die IP-Adresse des Exit Nodes finden Sie mit <code>tailscale ip -4</code> auf dem entsprechenden Gerät. Weitere Details in der <a href="https://tailscale.com/docs/features/exit-nodes" target="_blank">offiziellen Exit-Node-Dokumentation</a>.</p>
                    </div>
                    `,
                    htmlEn: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    ...
                    <p class="mt-3">All internet traffic from the client is now routed through the exit node. Find the exit node's IP with <code>tailscale ip -4</code> on that device. See the <a href="https://tailscale.com/docs/features/exit-nodes" target="_blank">official exit node documentation</a> for details.</p>
                    </div>
                    `
                }
            ]
        ...
        /* ============ 4. ACLs ============ */
        {
            ...
            introDe: 'Tailscale verwendet ein <strong>deny-by-default</strong>-Modell für die Zugriffskontrolle. ACLs werden in der <strong>Tailnet Policy File</strong> mit huJSON-Syntax definiert und auf jedem Gerät lokal durchgesetzt. Tailscale empfiehlt mittlerweile die Migration zu <strong>Grants</strong>, einer moderneren Syntax mit erweiterten Möglichkeiten[citation:6].',
            introEn: 'Tailscale uses a <strong>deny-by-default</strong> model for access control. ACLs are defined in the <strong>tailnet policy file</strong> using huJSON syntax and enforced locally on each device. Tailscale now recommends migrating to <strong>Grants</strong>, a more modern syntax with additional capabilities[citation:6].',
            subtopics: [
                {
                    id: 'subsection4_1',
                    ...
                    <p class="text-xs text-[var(--text-muted)] mt-2">Wenn Sie keine ACLs definieren, gilt die <strong>Default Allow All Policy</strong> – alle Geräte können auf alle zugreifen. Um allen Verkehr zu verbieten, verwenden Sie ein leeres <code>acls</code>-Objekt[citation:6].</p>
                    `,
                    htmlEn: `
                    <div class="overflow-x-auto w-full">
                    <table class="wikitable">
                    ...
                    <p class="text-xs text-[var(--text-muted)] mt-2">If you don't define any ACLs, the <strong>default allow all policy</strong> applies – all devices can access all others. To deny all traffic, use an empty <code>acls</code> object[citation:6].</p>
                    `
                },
                {
                    id: 'subsection4_2',
  ...
  "acls": [
    {
      ...
      "dst": ["tag:frontend:*"]
                    ...
                    <p class="mt-3">ACLs können in der Admin-Konsole unter <strong>Access Controls</strong> bearbeitet werden. Weitere Beispiele finden Sie in den <a href="https://tailscale.com/docs/reference/examples/acls" target="_blank">offiziellen ACL-Beispielen</a>.</p>
                    </div>
                    `,
                    htmlEn: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
  ...
  "acls": [
    {
      ...
      "dst": ["tag:frontend:*"]
                    ...
                    <p class="mt-3">ACLs can be edited in the admin console under <strong>Access Controls</strong>. See the <a href="https://tailscale.com/docs/reference/examples/acls" target="_blank">official ACL examples</a> for more.</p>
                    </div>
                    `
                }
            ]
  ...
  tailscale/tailscale:latest</pre>
                    ...
                    <li><code>TS_EXTRA_ARGS</code>: Zusätzliche <code>tailscale up</code>-Flags[citation:8].</li>
                    </ul>
                    </div>
                    `,
                    htmlEn: `
  ...
  tailscale/tailscale:latest</pre>
                    ...
                    <li><code>TS_EXTRA_ARGS</code>: Additional <code>tailscale up</code> flags[citation:8].</li>
                    </ul>
                    </div>
                    `
                },
                {
                    ...
                    htmlDe: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    <p class="mb-2"><strong>Sidecar-Pattern: Tailscale + Anwendung</strong></p>
                    <div class="jf-code">
...
volumes:
  ts-state:</pre>
                    ...
                    <p class="mt-3">Die Anwendung teilt sich den Netzwerk-Namespace mit dem Tailscale-Sidecar und ist dadurch über die Tailscale-IP erreichbar[citation:19].</p>
                    </div>
                    `,
                    htmlEn: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    <p class="mb-2"><strong>Sidecar pattern: Tailscale + application</strong></p>
                    <div class="jf-code">
                        <pre class="jf-code-inner">services:
  tailscale:
    image: tailscale/tailscale:latest
...
volumes:
  ts-state:</pre>
                    ...
                    <p class="mt-3">The application shares the network namespace with the Tailscale sidecar and is reachable via the Tailscale IP[citation:19].</p>
                    </div>
                    `
                }
            ]
        ...
        /* ============ 6. KONFIGURATION ============ */
        {
            ...
            introDe: 'Die Tailscale CLI bietet umfangreiche Steuerungsmöglichkeiten. Wichtige Befehle umfassen <code>tailscale up</code> (Verbindung herstellen), <code>tailscale status</code> (Netzwerkstatus), <code>tailscale ip</code> (IP-Adressen) und <code>tailscale ping</code> (Konnektivität testen). Weitere Details in der <a href="https://tailscale.com/kb/1080/cli" target="_blank">CLI-Referenz</a>.',
            introEn: 'The Tailscale CLI offers extensive control options. Key commands include <code>tailscale up</code> (connect), <code>tailscale status</code> (network status), <code>tailscale ip</code> (IP addresses), and <code>tailscale ping</code> (test connectivity). See the <a href="https://tailscale.com/kb/1080/cli" target="_blank">CLI reference</a> for details.',
            subtopics: [
                {
                    id: 'subsection6_1',
                    ...
                    <tr><td><code>tailscale ip -4</code></td><td class="text-[var(--text-muted)]">Eigene Tailscale IPv4-Adresse anzeigen.</td></tr>
                    ...
                    <tr><td><code>tailscale ip -4</code></td><td class="text-[var(--text-muted)]">Show your own Tailscale IPv4 address.</td></tr>
                ...
                },
                {
                    ...
                    <p class="mt-3"><strong>Warnung:</strong> Das Deaktivieren der Key Expiry reduziert die Sicherheit. Nur für vertrauenswürdige Geräte verwenden[citation:10].</p>
                    </div>
                    `,
                    htmlEn: `
                    <div class="bg-[var(--panel-color)] p-3 border border-[var(--panel-border)] rounded text-xs text-[var(--text-muted)]" style="box-shadow: var(--control-shadow);">
                    ...
                    <p class="mt-3"><strong>Warning:</strong> Disabling key expiry reduces security. Only do this for trusted devices[citation:10].</p>
                    </div>
                    `
                }
            ]
        ...
        /* ============ TLDR ============ */
        {
            ...
            titleDe: 'TLDR',
            titleEn: 'TLDR',
            introDe: 'Die wichtigsten Tailscale-Aspekte auf einen Blick.',
            introEn: 'The key Tailscale aspects at a glance.',
            subtopics: [
                {
                    id: 'tldr-grid',
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 mt-2">
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                            ...
                            <p class="text-xs text-[var(--text-muted)] leading-relaxed">Identity-basierte Konnektivität statt IP-basierter Regeln. WireGuard-Verschlüsselung, deny-by-default ACLs.</p>
                        </div>
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                        ...
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                            ...
                            <p class="text-xs text-[var(--text-muted)] leading-relaxed">Zugriff auf ganze Subnetze ohne Tailscale auf Zielgeräten. Exit Nodes für gesamten Internet-Traffic.</p>
                        </div>
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                            ...
                            <p class="text-xs text-[var(--text-muted)] leading-relaxed">Sidecar-Pattern für Container, Kubernetes Operator für Cluster-Integration. Auth Keys für Automatisierung.</p>
                        </div>
                    </div>
                    `,
                    htmlEn: `
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 mt-2">
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                            <div class="flex items-center gap-3 mb-3 font-semibold text-sm"><i class="fa-solid fa-shield-halved opacity-70"></i><span>1. Zero Trust & Identity</span></div>
                            <p class="text-xs text-[var(--text-muted)] leading-relaxed">Identity-based connectivity instead of IP-based rules. WireGuard encryption, deny-by-default ACLs.</p>
                        </div>
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                        ...
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                            <div class="flex items-center gap-3 mb-3 font-semibold text-sm"><i class="fa-solid fa-route opacity-70"></i><span>3. Subnet Router & Exit Nodes</span></div>
                            <p class="text-xs text-[var(--text-muted)] leading-relaxed">Access entire subnets without Tailscale on target devices. Exit nodes for all internet traffic.</p>
                        </div>
                        <div class="p-4 border border-[var(--panel-border)] rounded-lg bg-[var(--bg-color)] shadow-sm">
                            <div class="flex items-center gap-3 mb-3 font-semibold text-sm"><i class="fa-solid fa-docker opacity-70"></i><span>4. Docker & Kubernetes</span></div>
                            <p class="text-xs text-[var(--text-muted)] leading-relaxed">Sidecar pattern for containers, Kubernetes Operator for cluster integration. Auth keys for automation.</p>
                        </div>
                    </div>
                    `
                }
